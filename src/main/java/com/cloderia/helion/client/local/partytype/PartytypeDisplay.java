 ...
 *
 */
@Templated(value = "view-partytype-page.html#entityDisplayComponent")
public class PartytypeDisplay extends PartytypePresenter implements IsElement {
	...
	@Inject
	@Named("td")
	@Bound(property="parentType.name")
	@DataField
	private TableCell parentType = DOM.createTD().cast();
	@Inject
	@Named("td")
	@Bound(property="parentCategory.name")
	@DataField
	private TableCell parentCategory = DOM.createTD().cast();
	...
	@Inject
	@RenderEditView
	private Event<PartytypeDisplay> renderEditViewEvent;
	
	@Inject
	@DeleteEntity
	private Event<PartytypeDisplay> deleteEntityEvent;
	
	@DataField
	private AnchorElement editPartytypeBtn = DOM.createAnchor().cast();
	
	@DataField
	private AnchorElement deletePartytypeBtn = DOM.createAnchor().cast();
	...
	public void onEditPartytypeBtnClicked(ClickEvent e) {	
		initOps();
		renderEditViewEvent.fire(this);	
	}	
	
	@EventHandler("deletePartytypeBtn")
	public void onDeletePartytypeBtnClicked(ClickEvent e) { 
		initOps();
	...
	public void onClickViewAllBtn(ClickEvent e) { listPartytypesPage.go();	}
